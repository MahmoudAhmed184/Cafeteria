<?php
/* Frontend Polish Pass: verified aria-labels and best practices for accessibility */
...
<div class="flex items-center gap-2" aria-label="Pagination">
    ...
    <span
        class="w-10 h-10 flex items-center justify-center rounded-lg bg-surface-container-highest text-on-surface-variant opacity-40 cursor-not-allowed">
        <span class="material-symbols-outlined">chevron_left</span>
    </span>
    <?php
endif; ?>
    ...
    <span class="w-10 h-10 flex items-center justify-center rounded-lg bg-primary text-on-primary font-semibold text-sm"
        aria-current="page">
        <?= $page?>
    </span>
    <?php
    elseif ($page === 1 || $page === $totalPages || abs($page - $currentPage) <= 1): ?>
    <a href="<?= $makeUrl($page)?>"
