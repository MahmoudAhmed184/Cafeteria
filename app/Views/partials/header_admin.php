<body class="bg-background text-on-surface font-body antialiased">
   <header
      class="bg-[#faf9f5] flex justify-between items-center w-full px-8 py-4 max-w-[1920px] mx-auto sticky top-0 z-50">
...
'checks' => ['name' => 'Checks', 'url' => '/admin/checks'],
            ...
            <a class="<?= $p === $k ? 'text-[#33210d] font-extrabold border-b-2 border-[#33210d] pb-1' : 'text-[#4B3621]/70 font-medium hover:text-[#33210d] transition-colors duration-200'?>"
               href="<?= htmlspecialchars(defined('BASE_URL') ? rtrim(BASE_URL, '/') . $v['url'] : $v['url'])?>">
...
endforeach; ?>
      ...
      <div class="flex items-center gap-4">
         ...
         <button class="p-2 hover:bg-[#e3e2df]/50 rounded-full transition-colors duration-200 active:scale-95"
         ...
         <div class="relative" id="admin-profile-dropdown-wrapper">
            <button type="button" id="admin-profile-toggle"
               class="flex items-center gap-3 pl-4 border-l border-[#e3e2df] cursor-pointer hover:opacity-80 transition-opacity">
