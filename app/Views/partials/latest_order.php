...
<div class="latest-order-widget" id="latest-order-widget" aria-label="Latest order">
    ...
    <div class="latest-order-body">
        ...
        <p class="latest-order-date">
            <?= $e($latestOrder['created_at'] ?? '')?>
        </p>
        <p class="latest-order-status">Status: <strong>
                <?= $e($latestOrder['status'] ?? '')?>
            </strong></p>
        <p class="latest-order-total">Total: <strong>
                <?= $e(number_format((float)($latestOrder['total_amount'] ?? 0), 2))?> EGP
            </strong></p>
