                ...
                <thead class="bg-surface-container">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-xs font-semibold text-on-surface-variant uppercase tracking-wide">
                            Price</th>
                        <th
                            class="px-6 py-3 text-xs font-semibold text-on-surface-variant uppercase tracking-wide text-center">
                            Image</th>
                        <th
                            ...
                            <tr class="hover:bg-surface-container-low transition-colors duration-150">
                                <td class="px-6 py-4">
                                    <div class="flex flex-col">
                                        <span
                                            class="text-sm font-medium text-on-surface truncate"><?= $e($product['name'] ?? '') ?></span>
                                        <span
                                            class="text-xs text-on-surface-variant"><?= $e($product['category_name'] ?? '') ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-baseline gap-1">
                                        ...
                                        <span
                                            class="text-sm font-semibold text-on-surface"><?= $e(number_format((float) ($product['price'] ?? 0), 2)) ?></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                        ...
                                        <div class="w-12 h-12 rounded-md overflow-hidden bg-surface-container">
                                            ...
                                            <?php else: ?>
                                                <div class="w-full h-full flex items-center justify-center">
                                                    <span
                                                        class="material-symbols-outlined text-on-surface-variant text-[16px]">image</span>
                                                </div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-3 items-center">
                                        <?php if (!empty($product['is_available'])): ?>
                                            <form method="post"
                                                action="<?= defined('BASE_URL') ? BASE_URL . '/admin/products/toggle' : '/admin/products/toggle' ?>">
                                                <input type="hidden" name="csrf_token"
                                                    value="<?= htmlspecialchars((string) csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
                                                <input type="hidden" name="id" value="<?= $productId ?>">
                                                <button type="submit"
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-success-container border border-success/20 rounded-full text-xs font-medium text-success hover:bg-success-container/60 transition-colors">
                                        ...
                                        <?php else: ?>
                                            <form method="post"
                                                ...
                                                <input type="hidden" name="id" value="<?= $productId ?>">
                                                <button type="submit"
                                                    class="inline-flex items-center gap-1.5 px-2.5 py-1 bg-error-container/30 border border-error/20 rounded-full text-xs font-medium text-error hover:bg-error-container/50 transition-colors">
                                                    <span class="material-symbols-outlined text-[14px]"
                                        ...
                                        <a href="<?= defined('BASE_URL') ? BASE_URL . '/admin/products/edit?id=' . $productId : '/admin/products/edit?id=' . $productId ?>"
                                            ...
                                            aria-label="Edit <?= $e($product['name'] ?? '') ?>" title="Edit">
                                            <span class="material-symbols-outlined text-[20px]">edit</span>
                                        </a>
                                        <form method="post"
                                            action="<?= defined('BASE_URL') ? BASE_URL . '/admin/products/delete' : '/admin/products/delete' ?>">
                                            <input type="hidden" name="csrf_token"
                                                value="<?= htmlspecialchars((string) csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
                                            <input type="hidden" name="id" value="<?= $productId ?>">
                                            <button type="button"
                                                ...
                                                aria-label="Delete <?= $e($product['name'] ?? '') ?>" title="Delete"
