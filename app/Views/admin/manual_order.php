<?php /* Frontend Polish Pass: verified shared cart widget styling parity, lazy loading applied to images */
...
<div class="max-w-[1400px] mx-auto px-6 lg:px-8 py-8 grid grid-cols-12 gap-8 items-start">
            ...
            <form id="confirm-order-form" method="post"
                ...
                <div class="space-y-4 pt-2">
                    <div class="space-y-1.5">
                        <label class="block text-xs font-medium text-on-surface-variant uppercase tracking-wide" for="notes">Order Notes</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-medium text-on-surface-variant uppercase tracking-wide" for="room_no">Delivery Location</label>
    ...
    <section class="col-span-12 lg:col-span-8 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <!-- User Search -->
            <div class="relative">
                <span class="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-on-surface-variant text-[20px]">person_search</span>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 xl:grid-cols-4 gap-4" id="products-grid">
            ...
            <div class="group bg-surface-container-lowest rounded-lg border border-outline-variant/10 hover:border-outline-variant/40 transition-colors duration-150 cursor-pointer product-card"
                ...
                <div class="p-3 flex flex-col justify-between h-20">
                    <h3 class="text-sm font-medium text-on-surface leading-tight line-clamp-2"><?= $e($p['name']) ?></h3>
                    <div class="flex items-center justify-between">
                        <span class="text-xs text-on-surface-variant"><?= $e($p['category_name'] ?? 'Product') ?></span>
                        <span class="text-xs font-semibold text-secondary">EGP <?= $e(number_format((float)($p['price'] ?? 0), 2)) ?></span>
                    </div>
                </div>
                <button type="button"
...
<div id="add-success-overlay" class="fixed bottom-8 left-1/2 -translate-x-1/2 glass-panel border border-outline-variant/20 rounded-full px-5 py-2.5 flex items-center gap-2 z-[100] hidden">
