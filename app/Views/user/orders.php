...
<div class="py-8 px-6 lg:px-8 max-w-[1400px] mx-auto">
    ...
    <form method="GET" class="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="space-y-1.5">
            <label class="block text-xs font-medium text-on-surface-variant uppercase tracking-wide" for="date_to">Date
                to</label>
            <div class="relative">
                <span
    ...
    <div class="bg-surface-container-lowest rounded-lg border border-outline-variant/20 overflow-hidden">
                            ...
                            <tr class="group hover:bg-surface-container cursor-pointer order-toggle-row"
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-center gap-3">
                                        <span
                                            class="material-symbols-outlined text-on-surface-variant text-[18px] group-hover:text-primary transition-transform duration-200 order-toggle-icon inline-block">add</span>
                                ...
                                <td class="py-4 px-6">
                                    <div class="flex items-baseline gap-1">
                                        ...
                                        <span
                                            class="text-sm font-semibold text-on-surface"><?= $e(number_format((float) ($order['total_amount'] ?? 0), 2)) ?></span>
                                    </div>
                                </td>
                                <td class="py-4 px-6 text-right">
                                    <?php if ($status === 'Processing'): ?>
                                        <form method="post"
                                            action="<?= defined('BASE_URL') ? BASE_URL . '/orders/cancel' : '/orders/cancel' ?>">
                                        ...
                                        <?php if (!empty($order['items']) && is_array($order['items'])): ?>
                                            <div class="flex flex-wrap gap-4">
                                                <?php foreach ($order['items'] as $item): ?>
                                                    <div
                                                        class="flex items-center gap-3 bg-surface-container-lowest p-3 rounded-lg border border-outline-variant/15 min-w-[160px]">
                                                        <div
                                                            class="relative w-12 h-12 rounded-md overflow-hidden flex-shrink-0 bg-surface-container-highest">
                                                            ...
                                                            <?php else: ?>
                                                                <div class="w-full h-full flex items-center justify-center">
                                                                    <span
                                                                        class="material-symbols-outlined text-on-surface-variant text-[18px]">coffee</span>
                                                                </div>
                                                            <?php endif; ?>
                                                        </div>
                                                        <div>
                                                            ...
                                                            <p class="text-xs text-on-surface-variant">Qty:
                                                                <?= (int) ($item['quantity'] ?? 0) ?>
                                                            </p>
                                                        </div>
                                                    </div>
        ...
        <footer class="bg-surface-container px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            if (!empty($_GET['date_from']))
                $basePath .= '?date_from=' . urlencode($_GET['date_from']) . '&date_to=' . urlencode($_GET['date_to'] ?? '');
